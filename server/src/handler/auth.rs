...
use crate::model::reponse::UserData;
...
pub async fn auth(
    ...
    let access_token_details =
        match token::verify_jwt_token(data.conf.access_token.access_token_public_key.to_owned(), &access_token) {
    ...
    let user=sqlx::query_as(get_sql).bind(user_id_uuid.to_string()).fetch_optional(&data.db)
        .await
        .map_err(|e| {
            let error_response = ErrorResponse {
                status: "fail",
